...
**Paper-baseline conditions**: this experiment runs with the un-merged stacks
(1 mm equivalent-k `MOLD` gap between the two stacks on each edge) and the
synthetic hotspot GPU power map (p2a = 1.68), exactly like
[`EXP_DRAM_B`](../EXP_DRAM_B/) — its pure-DRAM anchor (141.7 °C). Use
`--merged --power-map uniform` for the merged/optimized conditions
(= `EXP_NVDRAM_S`).
