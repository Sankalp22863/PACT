...
FREQ_POWER = 300.0                     # 0.5x GPU frequency -> 300 W (paper: only compute dynamic
                                       # power scales, L2/interconnect fixed; NOT linear 0.5*414=207)
SUBSTRATE_UM = 1500                    # GPU-Si substrate (calibration knob, see experiment.config)
VOID_UM = 5000                         # effective mold-void width merge converts (calibrates merge to -17 C)
GRID_PREFIX = "hybrid.grid.steady"

# NVDRAM_baseline_no_freq: same as NVDRAM_baseline3 but WITHOUT the GPU frequency-scaling
# optimization — the GPU stays at full power (414 W) through thermal-silicon.
STAGES = [
    ...
    ("4_thermal_si",   "+ Thermal-silicon optimization", ["--no-base-die", "--merge-stacks", "--thin-top-die",
                                                          "--optimized"]),
]
...
def main():
    ...
    ap.add_argument("--nv-tiers", type=int, default=0)
    ap.add_argument("--dram-tiers", type=int, default=12)
    ap.add_argument("--label", default="All-DRAM: 12 DRAM (30x22 die)")
    args = ap.parse_args()
