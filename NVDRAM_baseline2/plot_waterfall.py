...
GPU_C  = "#c0392b"   # GPU compute die (all-DRAM red)
DRAM_C = "#2c7fb8"   # bottom-most DRAM tier (steel blue)
DROP   = "#2b8a3e"
INK    = "#2b2b2b"
MUTE   = "#8a8a8a"
...
def main():
    ...
    staircase(ax, dram, DRAM_C, -14, "Bottom-most DRAM tier (peak)")

    ax.text(0.015, 0.05,
            f"GPU {gpu[0]:.1f}→{gpu[-1]:.1f} °C   |   bottom-DRAM {dram[0]:.1f}→{dram[-1]:.1f} °C   |   "
            f"all-DRAM, 30×22 mm die",
    ...
    ax.set_title("All-DRAM STCO Thermal Waterfall — 30 × 22 mm die (paper-faithful layout)\n"
                 "GPU compute-die peak + bottom-most DRAM tier; calibrated to the "
                 "reference all-DRAM 3D baseline (141.7 °C)",
